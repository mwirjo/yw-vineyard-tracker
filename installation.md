...
2. Noem het bestand exact: `tailwind.config.js` *(verwijder de `.txt` extensie)*.
...
1. Klik met de rechtermuisknop in de hoofdmap ➔ **Nieuw** ➔ **Tekstdocument**.
2. Noem het bestand exact: `postcss.config.js` *(verwijder de `.txt` extensie)*.
3. Open het bestand en plak deze code erin:
```javascript
export default {
  plugins: {
...
*   `components` *(voor alle UI-schermen)*
*   `context` *(voor de globale game-state logica)*
